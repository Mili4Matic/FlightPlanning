...
# Output files
OUTPUT_GRAPH_FILE = '../../outputs/graphs/waypoints_graph_LEMD-UUEE_alter.gml'
OUTPUT_PLOT_FILE = '../../outputs/graphs/waypoints_graph_LEMD-UUEE_alter.png'
...
def plot_graph(G, output_plot_file):
    """Plot the graph on top of a geographic map and save the figure."""
    ...
    for u, v, data in G.edges(data=True):
        x = [pos[u][0], pos[v][0]]
        y = [pos[u][1], pos[v][1]]
        ax.plot(x, y, color='gray', linewidth=0.3, alpha=0.5, transform=ccrs.PlateCarree())

    # Draw nodes
    node_lons = [pos[n][0] for n in G.nodes()]
    node_lats = [pos[n][1] for n in G.nodes()]
    ax.scatter(node_lons, node_lats, s=5, c='steelblue', zorder=5, transform=ccrs.PlateCarree())

    ax.set_title('Airspace Graph (Waypoints and Airways)', fontsize=14)
